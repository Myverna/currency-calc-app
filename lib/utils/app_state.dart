...
class AppState extends ChangeNotifier {
  ...
  // Calculator methods - FIXED PERCENT BUG
  void calculatorInput(String input) {
    if (input == 'C') {
      ...
      // FIXED: Percent works immediately
      final current = double.parse(calculatorDisplay);
      calculatorDisplay = (current / 100).toString();
      _formatDisplay();
    } else if (input == '.') {
  ...
  void _formatDisplay() {
    final value = double.parse(calculatorDisplay);
    if (value == value.truncateToDouble()) {
      calculatorDisplay = value.toInt().toString();
    } else {
      calculatorDisplay = value.toStringAsFixed(2);
    }
  }
}